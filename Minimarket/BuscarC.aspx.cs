...
{
    public partial class BuscarC : System.Web.UI.Page
    {
        ...
        public void listar()
        {
            ...
            {
                ConectarBD();
                if (CbxBuscarC.SelectedIndex == 0)
                {
                    MessageBox.Show("Debe elegir una opcion");
                }
                else
                {
                    Tabb = new SqlCommand("select * from Producto where TipoProducto=@TipoProducto", Conn);
                    Tabb.Parameters.AddWithValue("TipoProducto", CbxBuscarC.SelectedIndex+1);
            ...
            {
                MessageBox.Show(EE.Message, "Base de Datos", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
        }
